// DSA 25 Patterns — Hard Edition Lab Controller
...
function getDataset() {
  if (typeof SUPER_HARD_DATA !== "undefined" && Array.isArray(SUPER_HARD_DATA)) return SUPER_HARD_DATA;
  if (typeof window !== "undefined" && Array.isArray(window.SUPER_HARD_DATA)) return window.SUPER_HARD_DATA;
  return [];
}
...
function render() {
  ...
  const diffClass = (p.difficulty || "hard").toLowerCase();
  ...
  <section class="card">
    <div class="card-meta-row">
      ...
      <span class="counter">Problem ${current + 1}/75</span>
    </div>
...
function step() {
  ...
  if (t === "Sliding Window") { let a = [..."abcabcbb"]; S.l ??= 0; S.r ??= -1; S.set ??= new Set(); if (S.r >= a.length - 1) return msg("Done."); S.r++; let c = a[S.r]; while (S.set.has(c)) S.set.delete(a[S.l++]); S.set.add(c); draw(); msg(`Add '${c}'; shrink left until valid.`); return }
  ...
  if (t === "K-Way Merge") { let L = [[1, 4, 7], [2, 5, 8], [3, 6, 9]], h = S.heads ?? [0, 0, 0]; S.out ??= []; let best = Infinity, bi = -1; for (let j = 0; j < 3; j++) { let k = S.heads[j]; if (k < 3 && L[j][k] < best) { best = L[j][k]; bi = j } } if (bi < 0) return msg("Merged."); S.out.push(best); S.heads[bi]++; draw(); msg(`Pop ${best}; advance source ${bi + 1}.`); return }
  ...
  if (t === "Dijkstra's Algorithm") { S.d ??= { A: 0, B: Infinity, C: Infinity }; S.done ??= []; if (!S.done.includes("A")) { S.d.B = 2; S.d.C = 5; S.done.push("A") } else if (!S.done.includes("B")) { S.d.C = 3; S.done.push("B") } else if (!S.done.includes("C")) S.done.push("C"); else return msg("Distances finalized."); draw(); msg("Relax edges from closest node."); return }
  if (t === "1D DP") { let a = [2, 7, 9, 3, 1]; S.dp ??= [0, 2]; S.i ??= 2; if (S.i >= a.length) return msg("DP complete."); S.dp[S.i] = Math.max(S.dp[S.i - 1], S.dp[S.i - 2] + a[S.i]); S.i++; draw(); msg("Compute next state from previous."); return }
  if (t === "2D DP") { let g = [[1, 3, 1], [1, 5, 1], [4, 2, 1]]; S.d ??= [[1, null, null], [null, null, null], [null, null, null]]; S.i ??= 0; S.j ??= 1; if (S.i >= 3) return msg("DP complete."); let i = S.i, j = S.j; S.d[i][j] = g[i][j] + Math.min(S.d[i - 1]?.[j] ?? Infinity, S.d[i]?.[j - 1] ?? Infinity); if (j === 2) { S.i++; S.j = 0 } else S.j++; draw(); msg("Fill from top/left states."); return }
  if (t === "Backtracking") { let c = [1, 2]; S.path ??= []; S.outs ??= []; if (S.path.length === 2) { S.outs.push([...S.path]); S.path.pop(); draw(); msg("Record solution, backtrack."); return } S.path.push(c[S.path.length]); draw(); msg("Choose → explore → undo."); return }
}
